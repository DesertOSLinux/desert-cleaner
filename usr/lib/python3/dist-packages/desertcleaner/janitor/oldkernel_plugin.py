...
from desertcleaner.gui.gtk import set_busy, unset_busy
from desertcleaner.janitor import JanitorPlugin, PackageObject
from desertcleaner.settings.debug import get_traceback, log_func
from desertcleaner.utils.package import AptWorker

log = logging.getLogger('OldKernelPlugin')


class OldKernelPlugin(JanitorPlugin):
    __title__ = _('Старые ядра')
    __category__ = 'system'
    ...
    def is_old_kernel_package(self, pkg):
        basenames = ['linux-image', 'linux-image-extra', 'linux-headers',
                     'linux-image-debug', 'linux-desert-modules', 'linux-modules',
